...
def evaluate_lenet5(learning_rate=0.02, n_epochs=100, emb_size=300, batch_size=50, filter_size=[3], maxSentLen=300, hidden_size=[300,300]):
    ...
    U_a = create_ensemble_para(rng, 8, hidden_size[0]) # the weight matrix hidden_size*2
    LR_b = theano.shared(value=np.zeros((8,),dtype=theano.config.floatX),name='LR_b', borrow=True)  #bias for each target class
    LR_para=[U_a, LR_b]
    layer_LR=LogisticRegression(rng, input=sent_embeddings, n_in=hidden_size[0], n_out=8, W=U_a, b=LR_b) #basically it is a multiplication between weight matrix and input feature vector
