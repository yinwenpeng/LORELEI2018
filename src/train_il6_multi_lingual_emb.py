...
def evaluate_lenet5(learning_rate=0.02, n_epochs=7, il5_emb_size=300, train_ratio=0.8,emb_size=300, batch_size=50, filter_size=[3,5], max_il5_phrase_len=3, max_en_phrase_len=5, hidden_size=300):
    ...
    il5_word2vec=load_fasttext_word2vec_given_file(root+'mono-lingual-il6-xinli.vec',300)
    ...
    source_ids, source_masks, target_ids, target_masks, il5_word2id, english_word2id = load_trainingdata_il6(root, english_vocab, il5_vocab, max_il5_phrase_len, max_en_phrase_len)
    ...
    train_target_mask = np.asarray(target_masks[:train_size], dtype=theano.config.floatX)
    ...
    dev_target_mask = np.asarray(target_masks[-dev_size:], dtype=theano.config.floatX)
    ...
    # en_rand_values_row_sum = np.sqrt(np.square(en_rand_values).sum(axis=1))
    # en_rand_values = en_rand_values / en_rand_values_row_sum[:, np.newaxis]
    ...
    il5_rand_values_row_sum = np.sqrt(np.square(il5_rand_values).sum(axis=1))
    il5_rand_values = il5_rand_values / il5_rand_values_row_sum[:, np.newaxis]
    ...
    HL_layer_params = [HL_layer_W1, HL_layer_b1, HL_layer_W2, HL_layer_b2, HL_layer_W3, HL_layer_b3]
    #doc, by pos
    ...
    HL_layer_3=HiddenLayer(rng, input=HL_layer_2.output, n_in=emb_size, n_out=emb_size, W=HL_layer_W3, b=HL_layer_b3, activation=T.tanh)
    ...
    loss = 1.0-batch_cosine


    params = HL_layer_params   # put all model parameters together
    cost=loss+1e-4*((HL_layer_W1**2).sum()+(HL_layer_W2**2).sum()+(HL_layer_W3**2).sum())
