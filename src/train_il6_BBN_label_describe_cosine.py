...
from load_data import  load_fasttext_multiple_word2vec_given_file,load_word2vec_to_init, load_il6_with_BBN,load_SF_type_descriptions
...
def evaluate_lenet5(learning_rate=0.01, n_epochs=100, emb_size=40, batch_size=50, describ_max_len=20, type_size=12,filter_size=[3,5], maxSentLen=100, hidden_size=[300,300]):
    ...
    cosine_scores = normalize_matrix_rowwise(bow_mean_emb).dot(normalize_matrix_rowwise(bow_mean_des).T)
    ...
    testing
    '''
    binarize_prob = T.where(cosine_scores > 0.0, 1, 0)
