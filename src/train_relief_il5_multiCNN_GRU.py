...
def evaluate_lenet5(learning_rate=0.01, n_epochs=100, emb_size=40, batch_size=50, filter_size=[3,5], maxSentLen=100, hidden_size=[300,300]):
    ...
    testing
    '''
    ensemble_NN_scores = T.max(T.concatenate([att_score_matrix.dimshuffle('x',0,1), score_matrix.dimshuffle('x',0,1)],axis=0),axis=0)
    binarize_prob = T.where( ensemble_NN_scores > 0.3, 1, 0)
