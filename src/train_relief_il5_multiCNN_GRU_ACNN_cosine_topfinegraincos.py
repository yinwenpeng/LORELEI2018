...
from load_data import  load_fasttext_multiple_word2vec_given_file,load_reliefweb_il5_12_multilabel,load_word2vec_to_init, load_BBN_multi_labels_dataset,load_SF_type_descriptions
from common_functions import normalize_matrix_rowwise,normalize_tensor3_colwise,store_model_to_file,Conv_with_Mask, create_conv_para, average_f1_two_array_by_col, average_f1_two_array_by_col_firstK,create_HiddenLayer_para, create_ensemble_para, cosine_matrix1_matrix2_rowwise, Diversify_Reg, GRU_Batch_Tensor_Input_with_Mask,Gradient_Cost_Para, Attentive_Conv_for_Pair, create_GRU_para,create_LSTM_para
...
def evaluate_lenet5(learning_rate=0.01, n_epochs=100, emb_size=40, batch_size=50, describ_max_len=20, type_size=12,filter_size=[3,5], maxSentLen=100, hidden_size=[300,300]):
    ...
    all_sentences, all_masks, all_labels, word2id=load_reliefweb_il5_12_multilabel(maxlen=maxSentLen)
    ...
    #classification layer, it is just mapping from a feature vector of size "hidden_size" to a vector of only two values: positive, negative
    U_att_a = create_ensemble_para(rng, 8, LR_att_input_size) # the weight matrix hidden_size*2
    LR_att_b = theano.shared(value=np.zeros((8,),dtype=theano.config.floatX),name='LR_b', borrow=True)  #bias for each target class
    LR_att_para=[U_att_a, LR_att_b]
    layer_att_LR=LogisticRegression(rng, input=LR_att_input, n_in=LR_att_input_size, n_out=8, W=U_att_a, b=LR_att_b) #basically it is a multiplication between weight matrix and input feature vector
    att_score_matrix = T.nnet.sigmoid(layer_att_LR.before_softmax)  #batch * 12
    sub_labels = labels[:,:8]
    att_prob_pos = T.where( sub_labels < 1, 1.0-att_score_matrix, att_score_matrix)
    ...
    #classification layer, it is just mapping from a feature vector of size "hidden_size" to a vector of only two values: positive, negative
    acnn_U_a = create_ensemble_para(rng, 8, acnn_LR_input_size) # the weight matrix hidden_size*2
    acnn_LR_b = theano.shared(value=np.zeros((8,),dtype=theano.config.floatX),name='LR_b', borrow=True)  #bias for each target class
    acnn_LR_para=[acnn_U_a, acnn_LR_b]
    acnn_layer_LR=LogisticRegression(rng, input=acnn_LR_input, n_in=acnn_LR_input_size, n_out=8, W=acnn_U_a, b=acnn_LR_b) #basically it is a multiplication between weight matrix and input feature vector
    acnn_score_matrix = T.nnet.sigmoid(acnn_layer_LR.before_softmax)  #batch * 12

    sub_labels = labels[:,:8]
    acnn_prob_pos = T.where( sub_labels < 1, 1.0-acnn_score_matrix, acnn_score_matrix)
    ...
    sum up prob, works
    '''
    ensemble_scores = 0.6*ensemble_NN_scores+0.4*0.5*(cosine_score_matrix[:,:8]+top_k_score_matrix[:,:8])
