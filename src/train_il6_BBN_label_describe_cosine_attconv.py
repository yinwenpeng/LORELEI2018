...
from common_functions import normalize_matrix_rowwise,store_model_to_file,Conv_with_Mask, create_conv_para, average_f1_two_array_by_col, Attentive_Conv_for_Pair,create_HiddenLayer_para, create_ensemble_para, cosine_matrix1_matrix2_rowwise, Diversify_Reg, Gradient_Cost_Para, GRU_Batch_Tensor_Input_with_Mask, create_LSTM_para



def evaluate_lenet5(learning_rate=0.01, n_epochs=100, emb_size=40, batch_size=50, describ_max_len=20, type_size=12,filter_size=[3,5], maxSentLen=100, hidden_size=[40,40]):
    ...
    conv_model2 = Conv_with_Mask(rng, input_tensor3=des_input,
    ...
    repeat_des_emb = T.repeat(sent_embeddings2.dimshuffle('x',0,1), batch_size, axis=0).reshape((batch_size*type_size, hidden_size[0]))
    repeat_des_bow = T.repeat(bow_mean_des.dimshuffle('x',0,1), batch_size, axis=0).reshape((batch_size*type_size, emb_size))


    score_input = T.concatenate([repeat_sent_emb,repeat_des_emb,repeat_des_bow,repeat_sent_emb*repeat_des_bow],axis=1)
    U_a = create_ensemble_para(rng, 1, 2*hidden_size[0]+2*emb_size) # the weight matrix hidden_size*2
    ...
    test_model = theano.function([sents_id_matrix, sents_mask,  des_id_matrix, des_mask], [binarize_prob,att_score_matrix], allow_input_downcast=True, on_unused_input='ignore')
